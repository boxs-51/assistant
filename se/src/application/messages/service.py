...
class CanonicalMessageService:
    """Atomic authority for canonical conversation messages and asset refs."""
        ...
        *,
        ...
        async with self.uow_factory() as uow:
            ...
            if session is None or session.user_id != owner_user_id:
                raise MessageNotFoundError("Session not found.")
            ...
            await uow.assets.delete_message_references(message_id)
            record = await uow.sessions.update_message_content(
                session_id,
                message_id,
                canonical,
            )
        ...
        *,
        ...
        for index, part in enumerate(validated.content):
            ...
            if part_type in _ASSET_PART_TYPES:
                ...
                canonical = self._canonical_attachment(
                    file_record,
                    blob_record,
                )
        ...
        canonical_message = GatewayMessage(
            role=role,
            content=normalized_parts,
        )
        return (
            canonical_message.model_dump(
                mode="json",
                exclude_none=True,
            )["content"],
            tuple(occurrences),
