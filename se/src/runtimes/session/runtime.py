...
class SessionRuntime(BaseRuntime):
    ...
    async def initialize(self, context: RuntimeContext) -> None:
        ...
        self.message_service = getattr(context.container, "message_service", None) or CanonicalMessageService(context.uow_factory)
        if self.uow_factory is None:
            raise ValueError("SessionRuntime requires uow_factory.")
        if self.message_service is None:
            raise ValueError("SessionRuntime requires CanonicalMessageService.")
    ...
    def _message_authority(self):
        if self.message_service is None:
            if self.uow_factory is None:
                raise RuntimeError("SessionRuntime message authority is unavailable.")
    ...
    async def _on_request_received(self, event: BaseEvent):
        ...
        try:
            ...
            await self._publish_persistence_failure(
                event,
                exc,
                status_code=403,
            )
    ...
    async def _persist_assistant_message(
        ...
        *,
        ...
        try:
            ...
            # F4 persists only canonical asset media. Provider-generated media
            # remains an F7 ingest concern; never store provider/base64/path
            # descriptors as durable conversation state.
            logger.warning(
