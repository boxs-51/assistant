...
@pytest.mark.asyncio
async def test_r8_g_g5_all_open_terminal_heads_do_not_resolve_task(tmp_path):
    engine, sessions, service, planner = await _setup(
        tmp_path,
        name="r8_g_g5_r9_boundary.sqlite",
    )
    try:
        source = await _seed_source(
            sessions,
            service,
            planner,
            task_id="task-r8-g-g5",
            fork_request_id="fork-r8-g-g5",
        )
        admission = await service.consume_fork_plan(source["plan"])
        store = _store(sessions)
        ...
        async with _Uow(sessions) as uow:
            ...
            source_terminal = await uow.agents.compare_and_set_execution(
                source["source_execution_id"],
        ...
        assert task.status in {"RUNNING", "WAITING"}
        assert durable_task.status in {"RUNNING", "WAITING"}
        assert durable_task.output is None
        assert durable_task.error is None
        assert budget.state.value == "OPEN"
        assert budget.active_branches == 2
        assert all(branch.resolution_state == "OPEN" for branch in branches)
    finally:
        await engine.dispose()
