...
class ChatResponse():
    ...
    def _parse_text_content(self, text: str) -> TextContent:
        """
        Hàm hỗ trợ phân tích văn bản thô từ Gemini và chuyển thành TextContent.
        Tự động nhận diện format dựa trên nội dung bên trong chuỗi.
        """
        if not text:
            return TextContent(data="", format="plain")
            
        # 1. Kiểm tra cấu trúc HTML
        html_pattern = re.compile(r"<[a-z][\s\S]*>.*</[a-z]?>||<[a-z].*/>", re.IGNORECASE)
        if html_pattern.search(text):
            return TextContent(data=text, format="structured")
            
        # 2. Kiểm tra cấu trúc Markdown
        markdown_pattern = re.compile(r"(^#+\s|\*\*.*?\*\*|\*.*?\*|^-\s|^[\d]+\.\s|\[.*?\]\(.*?\))", re.MULTILINE)
        if markdown_pattern.search(text):
            return TextContent(data=text, format="structured")
            
        # 3. Mặc định là text thuần
        return TextContent(data=text, format="plain")
    ...
    def _parse_gemini_parts_to_content(self, parts: List[Dict[str, Any]]) -> List[MessageContentPart]:
        ...
        for part in parts:
            ...
            if "text" in part:
                text_obj = self._parse_text_content(part["text"])
                ...
                try:
                    ...
                    if "candidates" in obj:
                        ...
                        if "content" in candidate and "parts" in candidate["content"]:
                            ...
                            # Thay vì chỉ lấy text, ta sẽ đóng gói toàn bộ các part vào metadata
                            # để client có thể tái cấu trúc đầy đủ.
                            if parsed_parts:
                                chunk_metadata["content_parts"] = [p.model_dump(exclude_none=True) for p in parsed_parts]

                            # Đồng thời, vẫn trích xuất text_delta để tương thích ngược với các client đơn giản
