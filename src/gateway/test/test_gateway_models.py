...
async def get_model_list(provider: str):
    """
    Hàm để kiểm tra endpoint lấy thông tin list các model từ một provider cụ thể.
    """
    print(f"\n--- 🧪 Testing GET /v1/models for provider: {provider} ---")
    
    url = f"{GATEWAY_URL}/v1/models?provider_name={provider}"
    headers = {
    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            print(f"✅ Success! Received model list for provider: {provider}")
            print("---------------------------------------------")
...
async def main():
    ...
    #await get_model_list(provider="gemini")
