...
from ...schemas import GatewayChatRequest, GatewayResponse, FinishReason
...
async def generate_stream_response(
    ...
    try:
        ...
        gateway_metrics.metrics.increment_failed(500)
...
async def handle_non_stream_response(
    ...
    with tracer.start_as_current_span("response_processing"):
        ...
        gateway_metrics.metrics.increment_success()
        latency = time.time() - start_time
        gateway_metrics.metrics.record_latency(
            gateway_response.provider, gateway_response.model, latency
        )
