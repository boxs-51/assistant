...
class SemanticCacheService:
    ...
    def __init__(self, vector_driver: VectorStorageDriver, embedding_service: EmbeddingService):
        self.vector_driver = vector_driver
        self.embedding_service = embedding_service
        logger.info("Semantic Cache service is ready.", vector_driver=vector_driver.__class__.__name__)
        ...
        try:
            with tracer.start_as_current_span("semantic_cache.get") as span: # type: ignore
                ...
                try:
                    ...
                    # 2. Truy vấn backend
                    entry, distance, miss_reason = await self.vector_driver.get(embedding)
    ...
    async def set(self, prompt: str, response: str, embedding: list[float]):
        ...
        with tracer.start_as_current_span("semantic_cache.set"):
            ...
            await self.vector_driver.set(entry)
    ...
    async def batch_set(self, items: List[Tuple[str, str, list[float]]]):
        ...
        for prompt, response, embedding in items:
            entry_id = self._get_prompt_id(prompt)
            metadata = CacheMetadata(prompt=prompt)
            entries.append(CacheEntry(id=entry_id, response=response, embedding=embedding, metadata=metadata)) # type: ignore
        if entries:
            await self.vector_driver.batch_set(entries)
