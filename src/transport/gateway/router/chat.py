...
from ....domain.schemas.session import Session
...
from ....transport.gateway.runtime import GatewayRuntime
.
.
.
async def generate_stream_response(
    ...
    kernel: RuntimeKernel = request.app.state.kernel
    gateway_runtime: GatewayRuntime = kernel.get_runtime('gateway')
    tool_manager = gateway_runtime.tool_manager
    capability_dispatcher = gateway_runtime.capability_dispatcher
    ...
    for i in range(max_tool_iterations):
        with tracer.start_as_current_span(f"stream_tool_loop_iteration_{i}") as loop_span:
            ...
            for tool_call in tool_calls:
                ...
                result_content = await capability_dispatcher.dispatch(
                    capability_name=tool_name,
                    params={'tool_name': tool_name, 'arguments': tool_args, 'identity': identity}
                )
