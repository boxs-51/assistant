...
**AE-R10 frozen runtime baseline inherited:** `bd6107014cd486cd8ab43010304d074ff74a1a0c`  
**Implementation branch:** `work/ptc-3b-7569e9ea`  
**Pull request:** #33  
**Status:** IMPLEMENTATION ACTIVE / RE-ANCHORED POST-R10 / CI PENDING
...
The PTC helper is stateless and request-scoped:
...
Every capability await MUST delegate to the R10-owned primitive:

```text
_probe_capability_with_budget(...)
```

Required sequencing:

1. bounded probe of base CHAT/CHAT_STREAM using the existing ProviderCallBudget;
2. if False, return ineligible immediately;
3. if no tools, return eligible;
4. bounded probe of TOOL_CALLING using the same ProviderCallBudget;
5. return that result.

The R10 primitive recomputes remaining time from the authoritative monotonic deadline for every probe, hard-fences the full await, cancels + drains owned probe work on logical timeout, rejects late success/error after deadline, preserves external cancellation, and consumes no retry token. PTC must not recreate that lifecycle.
...
- **M33** second TOOL_CALLING probe reuses the same ProviderCallBudget and is hard-bounded by the same absolute logical deadline; blocking probe work is cancelled/drained and no later provider/executor starts after expiry.
...
## 11. Post-R10 re-anchor record

The pre-hotfix staged PR #28 was closed unmerged after R10 post-merge findings
P1-A/P1-B/P1-C changed the authoritative probe/deadline contract.

PTC-3B was re-anchored from exact canonical main:

```text
main@7569e9eadf76eccee492b125c594833a5d4fcbb2
```

Current TDD checkpoints:

```text
test-only RED candidate: 11646d34908ea3df4500d1ef7a6ed2c2f1e78cff
bounded-probe production candidate: 52e1a408fac825a1179990b143da4bfb683e9d7e
```

PR #33 supersedes PR #28.
